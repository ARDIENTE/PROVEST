...
class AmenitiesAndFacilityRepo @Inject()(
  ...
  def getByIDs(projectID: UUID, subProjectID: UUID): Future[Seq[AmenitiesAndFacility]] =
    db.run(dao.Query.filter(r => r.subProjectID === subProjectID && r.projectID === projectID).result)
